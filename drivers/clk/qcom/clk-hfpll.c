...
static void __clk_hfpll_init_once(struct clk_hw *hw)
{
	...
	struct hfpll_config const *hc = h->c;
	...
	/* Write M and N only if EN_MN_VAL is enabled. */
	if (hc->mn_en) {
	...
	if (hd->user_reg) {
		...
		if(hc->post_div_val)
		regmap_update_bits(regmap, hd->user_reg, POST_DIV_MASK, post_div_val);
		
		unsigned long rate;
		rate = clk_hw_get_rate(hw);
		/* Pick the right VCO. */
		if (hd->user_vco_mask && rate > hd->low_vco_max_rate)
			regmap_update_bits(regmap, hd->user_reg,VCO_SEL_BIT,1)
	}
	if (hd->droop_reg)
