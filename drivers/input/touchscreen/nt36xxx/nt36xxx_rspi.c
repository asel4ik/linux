...
#if TOUCH_KEY_NUM > 0
const uint16_t nvt_spi_touch_key_array[TOUCH_KEY_NUM] = {
	KEY_BACK,
	KEY_HOME,
	KEY_MENU
};
 ...
 ******************************************************
 */
int32_t nvt_spi_check_fw_reset_state(enum NVT_RST_COMPLETE_STATE check_reset_state)
{
	uint8_t buf[8] = {0};
	int32_t ret = 0;
	int32_t retry = 0;
	int32_t retry_max = (check_reset_state == NVT_RESET_STATE_INIT) ? 10 : 50;
	struct nvt_ts_data *ts = nvt_spi_data;
 ...
 ******************************************************
 */
static int nvt_spi_gpio_config(struct nvt_ts_data *ts)
{
	int32_t ret = 0;
 ...
 ******************************************************
 */
static void nvt_spi_gpio_deconfig(struct nvt_ts_data *ts)
{
	if (gpio_is_valid(ts->irq_gpio))
