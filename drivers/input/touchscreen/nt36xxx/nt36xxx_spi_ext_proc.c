...
void nvt_spi_change_mode(uint8_t mode)
{
	...
	nvt_spi_set_page(ts->mmap->EVENT_BUF_ADDR | NVT_EVENT_MAP_HOST_CMD);
...
static void nvt_spi_read_mdata(uint32_t xdata_addr, uint32_t xdata_btn_addr)
{
	...
	nvt_spi_read(buf, (TOUCH_KEY_NUM * 2 + 1));
...
static int32_t nvt_spi_pen_diff_open(struct inode *inode, struct file *file)
{
	...
	if (nvt_spi_check_fw_reset_state(NVT_RESET_STATE_NORMAL_RUN)) {
		mutex_unlock(&ts->lock);
		return -EAGAIN;
	}
	...
	nvt_spi_check_fw_reset_state(NVT_RESET_STATE_NORMAL_RUN);
